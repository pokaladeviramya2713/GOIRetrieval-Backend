...
async def get_document_content(folder: str, filename: str, view: Optional[bool] = Query(None)):
    ...
    # Default: Disable raw file download access globally BUT allow it specifically 
    # for generated reports in the content/Reports folder
    if folder == "Reports":
        return FileResponse(
            path=file_path, 
...
def call_gemini_with_retry(prompt, user_message=None, temperature=0.7):
    ...
    # Prioritize 2.5-flash for speed and higher quota in 2026, followed by 2.5-pro
    models_to_try = [
        'gemini-2.5-flash',
        'gemini-2.5-pro',
        'gemini-2.0-flash',
    ...
    if not GEMINI_API_KEY:
        raise Exception("Gemini API Key is missing. Please set it in your environment.")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "403" in error_msg or "permission_denied" in error_msg:
                    print(f"[CRITICAL] API Key Error for {model}: {last_error}")
                    if "leaked" in error_msg:
                        raise Exception("CRITICAL: Your API key was blocked because it was leaked. Use a new one.")
                    break # Try next model or fail if key is just invalid for this model
                    
                if "429" in error_msg or "resource_exhausted" in error_msg:
                    wait_time = (attempt * 3) + 2
                    print(f"[QUOTA] {model} hit limit. Retrying in {wait_time}s... ({attempt+1}/{max_retries})")
                    time.sleep(wait_time)
                    continue
                
                print(f"[ERROR] {model} failed: {last_error}")
